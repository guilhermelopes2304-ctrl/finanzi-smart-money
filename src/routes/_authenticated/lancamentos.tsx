...
export const Route = createFileRoute("/_authenticated/lancamentos")({
  head: () => ({
    meta: [
      { title: "Histórico — FINANZZI" },
      { name: "description", content: "Veja e organize todas as suas receitas e despesas." },
    ],
  }),
...
function TransactionsPage() {
  ...
  const installmentSummary = useMemo(() => {
    const future = transactions.filter(
      (tx) => tx.type === "expense" && tx.date > todayISO() && Number(tx.installment_total ?? 0) > 1,
    );
    return {
    ...
    const filtered = transactions.filter((tx) => {
      ...
      if (search.trim()) {
        ...
        const searchable = [
          tx.description,
          payment,
          historyCategoryLabel(tx.description, tx.type, categories, tx.category_id),
  ...
  return (
    <div ref={rootRef} className="fin-screen fin-transactions w-full max-w-4xl mx-auto pb-10">
      <div data-fin-history-item>
        <PageHeader
          title="Meu histórico"
          subtitle={`${rows.length} ${rows.length === 1 ? "registro encontrado" : "registros encontrados"}`}
          action={<Button onClick={openNew}><Plus className="size-4" /> Lançar</Button>}
        />
      </div>
      ...
      <section data-fin-history-item className="mb-5 space-y-3">
        <div className="relative">
          ...
          <Input
            value={search}
            onChange={(event) => { setSearch(event.target.value); setVisibleCount(30); }}
            placeholder="Buscar Uber, Mercado, VR..."
            className="h-12 rounded-2xl border-border bg-card pl-11 pr-4 shadow-sm focus-visible:ring-1 focus-visible:ring-primary"
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
          {([
            ...
            <button
              key={value}
              type="button"
              onClick={() => { setType(value); setVisibleCount(30); }}
              className={cn(
                "fin-interactive fin-pressable shrink-0 rounded-full border px-4 py-2 text-xs font-bold transition-all",
                type === value
          ...
          <button
            ...
            className={cn(
              "fin-interactive fin-pressable ml-auto shrink-0 rounded-full border px-3 py-2 text-xs font-semibold",
              filtersOpen ? "border-primary bg-primary/10 text-primary" : "border-border bg-card text-muted-foreground hover:text-foreground",
            )}
          >
            <Filter className="mr-1 inline size-3.5" /> Mais filtros
          </button>
        </div>

        {filtersOpen && (
          <div className="surface-card grid gap-3 rounded-2xl p-4 animate-fin-enter sm:grid-cols-2 lg:grid-cols-3">
            <SelectLike label="Categoria">
              <select value={category} onChange={(event) => { setCategory(event.target.value); setVisibleCount(30); }} className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm text-foreground outline-none focus:border-primary">
                <option value="all">Todas as categorias</option>
                {categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            </SelectLike>
            <SelectLike label="Ordenar">
              <select value={sort} onChange={(event) => { setSort(event.target.value); setVisibleCount(30); }} className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm text-foreground outline-none focus:border-primary">
                <option value="date-desc">Mais novos primeiro</option>
            ...
            {hasActiveFilters && (
              <div className="flex items-end"><Button variant="outline" className="w-full" onClick={resetFilters}>Limpar filtros</Button></div>
            )}
          </div>
        )}
      </section>
      ...
      {isLoading ? (
        ...
        <div className="rounded-[28px] border border-border/70 bg-card p-10 text-center shadow-sm">
          ...
          <p className="mt-4 text-sm font-bold text-foreground">Nenhum lançamento encontrado</p>
          <p className="mt-1 text-xs text-muted-foreground">Tente outro termo ou limpe os filtros para ver seu histórico.</p>
          <Button variant="outline" className="mt-5" onClick={resetFilters}>Limpar filtros</Button>
        </div>
      ) : (
        <div className="space-y-7">
          {groupedRows.map(([date, group]) => (
            <section key={date} data-fin-history-item>
              <div className="mb-2 flex items-center justify-between px-1">
                <div className="flex min-w-0 items-center gap-2">
                  <CalendarDays className="size-3.5 shrink-0 text-muted-foreground" />
                  <h2 className="text-xs font-bold uppercase tracking-[0.08em] text-muted-foreground">{formatDay(date)}</h2>
                </div>
                <span className={cn("shrink-0 text-xs font-bold tabular-nums", group.total >= 0 ? "text-fin-success" : "text-foreground")}>
                  {group.total >= 0 ? "+" : "-"} {formatBRL(Math.abs(group.total))}
                </span>
              </div>
              ...
              <div className="overflow-hidden rounded-[24px] border border-border/70 bg-card shadow-sm">
                {group.items.map((tx) => {
                  ...
                  const payment = PAYMENT_METHODS.find((m) => m.value === tx.payment_method)?.label;
                  return (
                    <div key={tx.id} className="group flex min-w-0 items-center gap-3 border-b border-border/60 px-3 py-3.5 last:border-b-0 transition-colors hover:bg-primary/[0.035] sm:px-4">
                      <div className="shrink-0 rounded-full bg-neutral-900 p-0.5">
                        <TransactionBrandLogo description={title} />
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-bold text-foreground">{title}</p>
                        <div className="mt-1 flex min-w-0 items-center gap-1.5 text-[11px] text-muted-foreground">
                          <span className="truncate">{categoryLabel}</span>
                          {payment && <><span aria-hidden="true">•</span><span className="truncate">{payment}</span></>}
                        </div>
                      </div>
                      <div className="flex shrink-0 items-center gap-1">
                        <span className={cn("whitespace-nowrap text-sm font-extrabold tabular-nums", tx.type === "income" ? "text-fin-success" : "text-foreground")}>
                        ...
                        <div className="flex opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                          ...
                          <ConfirmDelete
                            ...
                            trigger={<Button size="icon" variant="ghost" aria-label="Excluir"><Trash2 className="size-4 text-danger" /></Button>}
                          />
                        </div>
                      </div>
                    </div>
          ...
          {visibleRows.length < rows.length && (
            <div className="flex justify-center pt-1">
              <Button variant="outline" onClick={() => setVisibleCount((count) => count + 30)}>Carregar mais</Button>
            </div>
          )}
        </div>
      )}
...
function SelectLike({ label, children }: { label: string; children: React.ReactNode }) {
