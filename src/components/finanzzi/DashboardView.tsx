    ...
    <div className="min-h-full bg-background text-foreground">
      <div className="mx-auto w-full max-w-3xl px-4 pb-28 pt-4 sm:px-6 sm:pb-10 sm:pt-7">
        <Reveal className="pb-5 sm:pb-7">
          <div className="relative overflow-hidden rounded-[28px] border border-border/70 bg-card px-5 py-6 shadow-[0_12px_45px_rgba(0,0,0,0.06)] sm:px-7 sm:py-8">
            <div className="pointer-events-none absolute inset-x-0 top-0 h-40 bg-[radial-gradient(circle_at_15%_0%,hsl(var(--primary)/0.16),transparent_55%),radial-gradient(circle_at_90%_20%,hsl(var(--primary)/0.08),transparent_45%)]" />
            <div className="relative">
              <p className="text-sm font-semibold text-muted-foreground">{getGreeting()}, {firstName}</p>
              <h1 className="mt-2 max-w-xl font-display text-[2.15rem] font-semibold leading-[0.98] tracking-[-0.065em] sm:text-5xl">Seu dinheiro, sem complicação.</h1>
              <p className="mt-3 max-w-lg text-sm leading-6 text-muted-foreground">Registre do seu jeito. O FINANZZI transforma isso em uma visão clara.</p>
              <div className="mt-5 inline-flex items-center gap-2 rounded-full border border-border/70 bg-background/70 px-3 py-1.5 text-xs font-semibold text-muted-foreground backdrop-blur">
                <span className="size-1.5 rounded-full bg-primary shadow-[0_0_0_4px_hsl(var(--primary)/0.12)]" />
                Visão atualizada em tempo real
              </div>
            </div>
          </div>
        </Reveal>
        ...
        <Reveal delay={85} className="mt-4">
          <section aria-labelledby="capacity-title">
            {isLoading ? <Skeleton className="h-40 rounded-[28px] bg-muted" /> : <div className={`relative overflow-hidden rounded-[28px] border p-5 shadow-[0_12px_40px_rgba(0,0,0,0.05)] sm:p-6 ${hasPressure ? "border-fin-danger/30 bg-fin-danger-soft" : "border-primary/15 bg-[linear-gradient(135deg,hsl(var(--primary)/0.15),hsl(var(--card))_48%,hsl(var(--primary)/0.06))]"}`}>
              ...
              <div className="relative flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.12em] text-muted-foreground">Disponível hoje</p>
                  <h2 id="capacity-title" className="mt-2 text-sm font-medium text-muted-foreground">Quanto você pode gastar sem apertar o mês</h2>
                </div>
              ...
              <p className="relative mt-5 font-display text-5xl font-semibold leading-none tracking-[-0.07em] text-foreground sm:text-6xl">{formatBRL(available)}</p>
        ...
        <Reveal delay={125} className="mt-8">
        ...
        <Reveal delay={165} className="mt-8">
