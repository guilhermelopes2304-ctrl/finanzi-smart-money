...
export function FinancialAssistant({ className }: { className?: string }) {
  ...
  if (!open) {
    return (
      <button
        ...
        aria-label="Abrir assistente financeiro Fin"
        className={cn("fixed bottom-24 right-4 z-40 grid size-14 place-items-center overflow-hidden rounded-full border-2 border-emerald-300/30 bg-[#062117] shadow-2xl shadow-black/30 transition-transform hover:scale-105 active:scale-95", className)}
      >
        <img src="/fin-assistente.png" alt="Fin" className="h-full w-full object-cover" loading="lazy" decoding="async" />
  ...
  return (
    <div className={cn("fixed bottom-24 right-3 z-50 w-[calc(100vw-1.5rem)] max-w-[390px] overflow-hidden rounded-3xl border border-emerald-300/20 bg-[#071a12] text-white shadow-2xl shadow-black/40 sm:right-4", className)}>
      <div className="flex items-center justify-between border-b border-white/10 bg-emerald-400/10 p-4">
        <div className="flex items-center gap-3"><div className="size-10 overflow-hidden rounded-xl bg-emerald-400/10"><img src="/fin-assistente.png" alt="Fin" className="h-full w-full object-cover" loading="lazy" decoding="async" /></div><div><div className="flex items-center gap-1.5 text-sm font-bold"><Sparkles className="size-3.5 text-emerald-300" /> Fin</div><p className="text-xs text-white/50">Seu assistente financeiro</p></div></div>
      ...
      <div className="max-h-[55vh] space-y-3 overflow-y-auto p-4">{messages.map((message, index) => <div key={`${message.from}-${index}`} className={cn("max-w-[88%] rounded-2xl px-3 py-2 text-sm leading-5", message.from === "fin" ? "bg-white/[0.07] text-white/80" : "ml-auto bg-emerald-400 text-[#032013] font-medium")}>{message.text}</div>)}</div>
