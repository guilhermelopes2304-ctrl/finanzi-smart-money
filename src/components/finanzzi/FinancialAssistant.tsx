...
export function FinancialAssistant({ className }: { className?: string }) {
  ...
  if (!open) return <button type="button" onClick={() => setOpen(true)} aria-label="Abrir assistente financeiro Fin" className={cn("fixed bottom-24 right-4 z-40 grid size-14 place-items-center overflow-hidden rounded-full border-2 border-[#1E293B]/30 bg-[#0A0F1D] shadow-2xl shadow-black/30 transition-transform hover:scale-105 active:scale-95", className)}><img src="/fin-assistente.png" alt="Fin" className="h-full w-full object-cover" loading="lazy" decoding="async" /><span className="absolute bottom-0 right-0 size-3 rounded-full border-2 border-[#0A0F1D] bg-[#39FF14]" /></button>;
  return <div className={cn("fixed bottom-24 right-3 z-50 w-[calc(100vw-1.5rem)] max-w-[390px] overflow-hidden rounded-3xl border border-[#1E293B]/20 bg-[#0A0F1D] text-white shadow-2xl shadow-black/40 sm:right-4", className)}><div className="flex items-center justify-between border-b border-[#1E293B]/10 bg-[#39FF14]/10 p-4"><div className="flex items-center gap-3"><div className="size-10 overflow-hidden rounded-xl bg-[#39FF14]/10"><img src="/fin-assistente.png" alt="Fin" className="h-full w-full object-cover" /></div><div><div className="flex items-center gap-1.5 text-sm font-bold"><Sparkles className="size-3.5 text-[#39FF14]" /> Fin</div><p className="text-xs text-[#94A3B8]/50">Seu assistente financeiro</p></div></div><button type="button" onClick={() => setOpen(false)} className="rounded-full p-2 text-[#94A3B8]/50 hover:bg-[#1E293B]/10" aria-label="Fechar"><X className="size-4" /></button></div><div className="max-h-[55vh] space-y-3 overflow-y-auto p-4">{messages.map((message, index) => <div key={`${message.from}-${index}`} className={cn("max-w-[88%] rounded-2xl px-3 py-2 text-sm leading-5", message.from === "fin" ? "bg-white/[0.07] text-[#94A3B8]/80" : "ml-auto bg-[#39FF14] text-[#0A0F1D] font-medium")}>{message.text}</div>)}</div><div className="border-t border-[#1E293B]/10 p-3"><div className="flex gap-2 rounded-2xl border border-[#1E293B]/10 bg-white/[0.04] p-1"><button type="button" onClick={listening ? undefined : startListening} className={cn("grid size-10 shrink-0 place-items-center rounded-xl", listening ? "bg-[#1E293B] text-[#FFFFFF]" : "bg-[#39FF14] text-[#0A0F1D]")} aria-label={listening ? "Ouvindo" : "Falar com o Fin"}>{listening ? <MicOff className="size-4" /> : <Mic className="size-4" />}</button><input value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void send(); }} placeholder={listening ? "Estou ouvindo..." : saving ? "Salvando lançamento..." : "Pergunte ou diga um lançamento..."} disabled={saving} className="min-w-0 flex-1 bg-transparent px-3 py-2 text-sm outline-none placeholder:text-[#94A3B8]/30" /><button type="button" onClick={() => void send()} disabled={saving} className="grid size-10 shrink-0 place-items-center rounded-xl bg-[#39FF14] text-[#0A0F1D] disabled:opacity-50" aria-label="Enviar"><Send className="size-4" /></button></div><div className="mt-2 flex flex-wrap gap-1.5"><button type="button" onClick={() => setInput("Posso gastar hoje?")} className="rounded-full bg-[#1E293B]/5 px-2.5 py-1 text-[11px] text-[#94A3B8]/55">Posso gastar?</button><button type="button" onClick={() => setInput("Como posso economizar?")} className="rounded-full bg-[#1E293B]/5 px-2.5 py-1 text-[11px] text-[#94A3B8]/55">Como economizar?</button><button type="button" onClick={() => setInput("Gastei 50 no mercado")} className="rounded-full bg-[#1E293B]/5 px-2.5 py-1 text-[11px] text-[#94A3B8]/55">Testar lançamento</button></div></div></div>;
}
