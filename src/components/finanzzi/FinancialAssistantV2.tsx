...
export function FinancialAssistant({ className }: { className?: string }) {
  ...
  useEffect(() => {
    if (open && messages.length === 0) {
      setMessages([
        {
          from: "fin",
          text: `Olá${profile?.name ? `, ${profile.name.split(" ")[0]}` : ""}. Posso ajudar você a entender seu dinheiro ou registrar algo. Escreva ou fale do seu jeito.`,
        },
      ]);
    }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b border-[#556070]/10 bg-[#FF5A1F]/10 p-4 sm:p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#F4F5F8]/50">Ajuda para entender e registrar</p>
          </div>
        </div>
        <button
          type="button"
      ...
      <div ref={scrollRef} className="max-h-[55vh] space-y-3 overflow-y-auto p-4 sm:p-5">
        ...
        {messages.length === 1 && !busy && !pending && (
          <div className="flex flex-wrap gap-2">
            {["Como está meu mês?", "O que vence esta semana?"].map((suggestion) => (
              <button
                key={suggestion}
                type="button"
        ...
        {busy && (
          <div className="flex items-center gap-2 rounded-2xl bg-white/[0.07] px-3 py-2 text-sm text-[#F4F5F8]/60">
      ...
      <div className="border-t border-[#556070]/10 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
        <div className="flex gap-2 rounded-2xl border border-[#556070]/10 bg-white/[0.04] p-1">
          <button
            ...
            className={cn(
              ...
              listening ? "bg-[#556070] text-[#FCFCF8]" : "bg-[#FF5A1F] text-[#111827]",
            )}
          ...
          <input
            ...
            className="min-w-0 flex-1 bg-transparent px-3 text-sm outline-none placeholder:text-[#F4F5F8]/30"
        ...
        <p className="mt-2 px-1 text-[10px] leading-4 text-[#F4F5F8]/35">
