    ...
    <div className="fixed inset-0 z-[80] lg:hidden" role="dialog" aria-modal="true" aria-label="Menu FINANZZI">
      ...
      >
        <div className="flex min-h-[72px] shrink-0 items-center justify-between border-b border-border px-5">
          <Link to="/dashboard" onClick={onClose} aria-label="FINANZZI">
            <Logo />
          </Link>
          <button
            type="button"
        ...
        <nav className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-5" style={{ WebkitOverflowScrolling: "touch" }}>
          ...
          <div className="space-y-1">
            {NAV.map((item) => (
              <NavItem key={item.to} item={item} active={activePathname === item.to} onNavigate={onClose} />
            ))}
          </div>
        </nav>
        <div className="shrink-0 border-t border-border px-4 py-4">
          <p className="px-2 text-xs text-muted-foreground">Sua vida financeira, em um só lugar.</p>
        </div>
      </aside>
    </div>
  );
    ...
    >
      ...
      <aside className={cn(
        ...
        <nav className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 py-5">
          <div className="space-y-1">
            {NAV.map((item) => <NavItem key={item.to} item={item} active={activePathname === item.to} collapsed={sidebarCollapsed} />)}
          </div>
        </nav>
        <div className="shrink-0 border-t border-border p-4" style={{ paddingBottom: "max(1rem, env(safe-area-inset-bottom))" }}>
          <Button
            className={cn("h-11 rounded-xl shadow-sm fin-interactive fin-pressable", sidebarCollapsed ? "w-11 justify-center px-0" : "w-full")}
            onClick={() => !visualReview && setTransactionOpen(true)}
      ...
      <div className={cn("flex min-h-0 min-w-0 flex-1 flex-col", desktopSidebarWidth)}>
        {!isHome && (
          <header
            className="relative z-20 flex shrink-0 items-center justify-between border-b border-border/60 bg-background/90 px-4 py-2.5 backdrop-blur-xl lg:hidden"
          ...
          >
            ...
            <button
              ...
              className="grid size-11 shrink-0 place-items-center rounded-full border border-border/70 bg-card text-foreground shadow-sm active:scale-95"
            >
        ...
        {isHome && (
          <button
            ...
            onClick={() => setDrawerOpen(true)}
            aria-label="Abrir menu"
            className="fixed left-4 z-50 grid size-11 place-items-center rounded-full border border-border/70 bg-card/80 text-foreground shadow-lg backdrop-blur-xl active:scale-95 lg:hidden"
        ...
        {isInternalTest && (
          <div className="shrink-0 border-b border-border bg-fin-brand-soft px-4 py-2 text-center text-[10px] font-bold uppercase tracking-[0.16em] text-fin-brand-hover">
        ...
        <main
          className={cn(
            "min-h-0 min-w-0 flex-1 overflow-hidden px-3 pt-3 sm:px-5 sm:pt-4 lg:px-8 lg:pt-7",
            isHome && "px-0 pt-0 sm:px-0 sm:pt-0 lg:px-0 lg:pt-0",
          )}
          style={{ paddingBottom: isHome ? 0 : "max(1rem, env(safe-area-inset-bottom))" }}
        >
