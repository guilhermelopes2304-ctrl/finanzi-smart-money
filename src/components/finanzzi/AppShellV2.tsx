...
export function AppShellV2({ children, visualReview = false }: { children: ReactNode; visualReview?: boolean }) {
  ...
  return <div data-fin-app-shell className="flex h-[100dvh] min-h-[100svh] w-full overflow-hidden bg-background text-foreground"><NavigationLoading />
    ...
    <div className={cn("flex min-h-0 min-w-0 flex-1 flex-col", desktopSidebarWidth)}>
      <header className="relative z-20 flex min-h-14 shrink-0 items-center border-b border-border/60 bg-background/90 px-4 backdrop-blur-xl lg:hidden" style={{ paddingTop: "max(0.5rem, env(safe-area-inset-top))" }}>
        <div className="grid w-full grid-cols-[44px_1fr_44px] items-center gap-2"><button type="button" onClick={() => setDrawerOpen(true)} aria-label="Abrir menu" className="grid size-11 place-items-center rounded-full border border-border/70 bg-card/80 text-foreground shadow-sm backdrop-blur-xl active:scale-95"><Menu className="size-5" /></button><Link to="/dashboard" aria-label="Ir para o início" className="flex min-w-0 items-center justify-center"><Logo /></Link><span aria-hidden="true" className="size-11" /></div>
      </header>
      ...
      <main className={cn("min-h-0 min-w-0 flex-1 overscroll-y-contain px-3 pt-4 sm:px-5 sm:pt-5 lg:px-8 lg:pt-7", isHome ? "overflow-hidden px-0 pt-0 sm:px-0 sm:pt-0 lg:px-0 lg:pt-0" : "overflow-y-auto")} style={{ paddingBottom: isHome ? 0 : "max(1rem, env(safe-area-inset-bottom))", WebkitOverflowScrolling: "touch" }}>
