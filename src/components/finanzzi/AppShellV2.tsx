...
import { BarChart3, CreditCard, Home, Plus, Settings, Sparkles, Wallet } from "lucide-react";
...
function MobileTabBar({ activePathname, onAdd }: { activePathname: string; onAdd: () => void }) {
  return <nav className="fin-mobile-tabbar fixed inset-x-3 bottom-2 z-[70] grid grid-cols-[1fr_1fr_auto_1fr_1fr] items-end gap-1 rounded-[24px] px-2 pb-[max(6px,env(safe-area-inset-bottom))] pt-2 lg:hidden" aria-label="Navegação principal">
...
export function AppShellV2({ children, visualReview = false }: { children: ReactNode; visualReview?: boolean }) {
  ...
  return <div data-fin-app-shell className="flex min-h-[100dvh] w-full overflow-hidden bg-background text-foreground">
    ...
    <div className="flex min-w-0 flex-1 flex-col lg:ml-[236px]">
      ...
      <main className={cn("min-h-0 min-w-0 flex-1", isHome ? "overflow-y-auto" : "overflow-y-auto px-3 py-5 sm:px-5 sm:py-6 lg:px-8 lg:py-7")} style={{ paddingBottom: isHome ? "calc(92px + env(safe-area-inset-bottom))" : "max(1rem, env(safe-area-inset-bottom))", WebkitOverflowScrolling: "touch" }}><MotionPage className={cn("mx-auto min-h-full max-w-7xl", isHome && "max-w-none")}>{children}</MotionPage></main>
    </div>
