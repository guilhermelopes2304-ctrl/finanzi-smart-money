    ...
    <div className="min-h-screen bg-background">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-background/95 pb-[env(safe-area-inset-bottom)] backdrop-blur lg:hidden">
        <div className="grid grid-cols-5 items-end px-1">
          ...
          <div className="flex justify-center">
            <button type="button" onClick={() => setTransactionOpen(true)} aria-label="Novo lançamento" className="gradient-brand -mt-7 grid size-14 place-items-center rounded-full text-white shadow-[var(--shadow-lift)] transition-transform active:scale-95"><Plus className="size-6" strokeWidth={2.5} /></button>
          </div>
          <button type="button" onClick={openAssistant} className="flex flex-col items-center gap-1 py-2.5 text-[10px] font-medium text-muted-foreground"><MessageCircle className="size-5" /><span>Fin</span></button>
      ...
      {moreOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          <button type="button" aria-label="Fechar menu" className="absolute inset-0 bg-black/45" onClick={() => setMoreOpen(false)} />
