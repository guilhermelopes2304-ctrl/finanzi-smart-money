...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-background text-foreground transition-colors">
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r border-border bg-card lg:flex"><div className="flex items-center justify-between border-b border-border/70 px-5 py-5"><Logo /><ThemeToggle /></div><nav className="flex-1 space-y-1 overflow-y-auto px-3 py-4">{NAV.map((item) => { const active = pathname === item.to; return <Link key={item.to} to={item.to} className={cn("flex min-h-11 items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors", active ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted hover:text-foreground")}><item.icon className="size-4" strokeWidth={active ? 2.2 : 1.9} />{item.label}</Link>; })}</nav><div className="space-y-2 border-t border-border/70 p-4"><Button className="h-11 w-full rounded-xl shadow-none" onClick={() => setTransactionOpen(true)}><Plus className="size-4" /> Novo lançamento</Button><button onClick={signOut} className="flex min-h-11 w-full items-center gap-2 rounded-xl px-3 py-2 text-sm text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"><LogOut className="size-4" /> Sair</button></div></aside>
    <header className="sticky top-0 z-20 border-b border-border/70 bg-background/90 px-4 py-2.5 backdrop-blur-xl lg:hidden"><div className="flex min-h-11 items-center justify-end gap-2"><ThemeToggle /><Link to="/configuracoes" aria-label="Abrir configurações" className="grid size-10 place-items-center rounded-full border border-border bg-card text-xs font-semibold text-primary transition-colors active:scale-95">{initials(profile?.name)}</Link></div></header>
