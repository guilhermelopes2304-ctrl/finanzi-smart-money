...
import { Bot, ChevronLeft, ChevronRight, CreditCard, History, Loader2, Menu, Plus, Send, Settings, Sparkles, Target, WalletCards } from "lucide-react";
...
export function HomeChat({ profile, transactions, isLoading = false }: HomeChatProps) {
  ...
  useEffect(() => {
    ...
    html.style.overflow = "hidden"; html.style.overscrollBehavior = "none"; html.style.height = "100%";
    body.style.overflow = "hidden"; body.style.overscrollBehavior = "none"; body.style.height = "100%"; body.style.position = "fixed"; body.style.width = "100%"; body.style.top = `-${scrollY}px`;
    window.scrollTo(0, 0);
    return () => { html.style.overflow = previous.htmlOverflow; html.style.overscrollBehavior = previous.htmlOverscroll; html.style.height = previous.htmlHeight; body.style.overflow = previous.bodyOverflow; body.style.overscrollBehavior = previous.bodyOverscroll; body.style.height = previous.bodyHeight; body.style.position = previous.bodyPosition; body.style.width = previous.bodyWidth; body.style.top = previous.bodyTop; window.scrollTo(0, scrollY); };
  }, []);
  ...
  function push(role: Message["role"], text: string) { setMessages((current) => [...current, { id: crypto.randomUUID(), role, text }]); }
  function useSuggestion(text: string) { setInput(text); requestAnimationFrame(() => inputRef.current?.focus()); }

  async function handleSend(value?: string) {
    const text = (value ?? input).trim();
    if (!text || busy) return;
    setInput(""); push("user", text);
    ...
    if (interpretation.intent === "record_transaction" && user && draft.amount > 0 && draft.confidence !== "low") {
      ...
      if (!transaction) { push("fin", "Entendi o valor, mas preciso de mais detalhes para registrar."); return; }
      setBusy(true);
      try {
        await saveTransaction({ userId: user.id, ...transaction }); invalidate();
      ...
      } catch { push("fin", "Não consegui salvar esse lançamento agora. Tente novamente em alguns segundos."); }
      finally { setBusy(false); }
      return;
    }
    if (interpretation.intent === "query_fin") {
      const normalized = interpretation.text.toLowerCase(); const period = buildPeriod("current", monthRange());
      if (/quanto.*gastei|gasto.*mês|gasto.*mes|resumo/.test(normalized)) { const totals = totalsFor(transactions, period); push("fin", `Neste mês você gastou ${formatBRL(totals.expense)}.\nEntradas: ${formatBRL(totals.income)}\nResultado: ${formatBRL(totals.balance)}`); return; }
      if (/onde|gastando mais|maior gasto/.test(normalized)) { const top = expensesByCategory(transactions, categories, period)[0]; push("fin", top ? `Sua maior categoria neste mês é ${top.name}: ${formatBRL(top.value)}.` : "Ainda não tenho despesas suficientes para encontrar um padrão."); return; }
    }
    if (/quanto.*dispon|saldo|tenho.*disponível|tenho.*disponivel/.test(text.toLowerCase())) { const capacity = spendCapacity({ accounts, transactions, bills, goals }); push("fin", `Sua margem disponível estimada é ${formatBRL(capacity.perDay)} por dia.`); return; }
  ...
  return (
    <section className="fixed inset-0 z-40 flex h-[100dvh] min-h-0 w-full touch-none overflow-hidden overscroll-none bg-background text-foreground lg:static lg:z-auto lg:h-[calc(100dvh-5rem)]">
      ...
      <div className="relative flex h-full min-h-0 w-full flex-col overflow-hidden px-4 pb-[max(8px,env(safe-area-inset-bottom))] pt-[max(14px,env(safe-area-inset-top))] sm:px-8 lg:pt-6">
        ...
        <main className="mx-auto flex min-h-0 w-full max-w-3xl flex-1 flex-col overflow-hidden py-4 sm:py-7">
          <div className={cn("shrink-0 text-center transition-all duration-300", messages.length ? "mb-5" : "mb-4")}>
            {messages.length === 0 ? <><p className="text-xs font-semibold tracking-[0.16em] text-primary">OLÁ, {firstName.toUpperCase()}</p><h1 className="mt-4 text-[2.15rem] font-semibold leading-[1.02] tracking-[-0.065em] sm:text-5xl">O que aconteceu<br className="sm:hidden" /> com seu dinheiro?</h1><p className="mx-auto mt-4 max-w-md text-sm leading-6 text-muted-foreground">Fale do seu jeito. Eu registro, organizo e te ajudo.</p></> : <p className="text-xs font-medium text-muted-foreground">Conversa com o FINANZZI</p>}
          </div>
          {messages.length === 0 && !isLoading && recent.length > 0 && <div className="mx-auto mb-4 flex shrink-0 max-w-xl flex-wrap justify-center gap-2">{suggestions.map((item) => <button key={item} type="button" disabled={busy} onClick={() => useSuggestion(item)} className="rounded-full border border-white/[0.09] bg-card/50 px-3.5 py-2 text-xs text-muted-foreground backdrop-blur-sm transition hover:border-primary/30 hover:bg-primary/[0.06] hover:text-foreground active:scale-[0.98]">{item}</button>)}</div>}
      ...
      <aside className={cn("fixed inset-y-0 left-0 z-[70] flex w-[min(86vw,320px)] flex-col border-r border-white/[0.08] bg-card/95 p-4 pb-[max(16px,env(safe-area-inset-bottom))] pt-[max(16px,env(safe-area-inset-top))] shadow-[20px_0_70px_rgba(0,0,0,0.35)] backdrop-blur-2xl transition-transform duration-300", sidebarOpen ? "translate-x-0" : "-translate-x-full")}>
