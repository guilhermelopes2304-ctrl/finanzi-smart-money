...
import { ChevronLeft, ChevronRight, CreditCard, History, Loader2, Menu, Plus, Send, Settings, Sparkles, Target, WalletCards } from "lucide-react";
...
import { FinanceResponseCard } from "./FinanceResponseCard";
...
const suggestions = ["Gastei R$ 45 no almoço", "Recebi R$ 2.000 de salário", "Quanto gastei este mês?"];
const nav = [
...
export function HomeChat({ profile, transactions, isLoading = false }: HomeChatProps) {
  ...
  useEffect(() => {
    ...
    const previous = { htmlOverflow: html.style.overflow, htmlOverscroll: html.style.overscrollBehavior, htmlHeight: html.style.height, bodyOverflow: body.style.overflow, bodyOverscroll: body.style.overscrollBehavior, bodyHeight: body.style.height, bodyPosition: body.style.position, bodyWidth: body.style.width, bodyTop: body.style.top };
  ...
  return (
    <section className="fixed inset-0 z-40 flex h-[100dvh] min-h-0 w-full touch-none overflow-hidden overscroll-none bg-background text-foreground lg:static lg:z-auto lg:h-[calc(100dvh-5rem)]">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_18%,hsl(var(--primary)/0.13),transparent_38%)]" />
      <div className="relative flex h-full min-h-0 w-full flex-col overflow-hidden px-4 pt-[max(10px,env(safe-area-inset-top))] sm:px-8 lg:pt-5">
        ...
        <main className="mx-auto flex min-h-0 w-full max-w-3xl flex-1 flex-col py-2 sm:py-5">
          <div className={cn("shrink-0 text-center transition-all duration-300", messages.length ? "mb-4" : "mb-5")}>
            {messages.length === 0 ? (
              <>
                <p className="text-xs font-semibold tracking-[0.16em] text-primary">OLÁ, {firstName.toUpperCase()}</p>
                <h1 className="mt-3 bg-gradient-to-r from-white via-neutral-200 to-neutral-500 bg-clip-text text-[2.1rem] font-semibold leading-[1.04] tracking-[-0.065em] text-transparent sm:text-5xl">O que aconteceu<br className="sm:hidden" /> com seu dinheiro?</h1>
                <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-muted-foreground">Fale do seu jeito. Eu registro, organizo e te ajudo.</p>
              </>
            ) : (
          ...
          {messages.length === 0 && !isLoading && (
            <div className="mx-auto mb-3 flex shrink-0 max-w-2xl flex-wrap justify-center gap-2">
              {suggestions.map((item) => (
                <button key={item} type="button" disabled={busy} onClick={() => useSuggestion(item)} className="rounded-full border border-white/[0.09] bg-card/60 px-3.5 py-2 text-xs text-muted-foreground backdrop-blur-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:bg-primary/[0.06] hover:text-foreground active:scale-[0.98]">{item}</button>
              ))}
            </div>
          )}
          ...
          <div className={cn("min-h-0 flex-1 overscroll-contain px-1 touch-pan-y", messages.length ? "overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" : "overflow-hidden")} aria-live="polite">
            <div className="space-y-3 pb-2">
              {messages.map((message) => message.role === "user" ? <div key={message.id} className="ml-auto max-w-[88%] rounded-[22px] rounded-tr-md bg-primary px-4 py-3 text-sm text-primary-foreground whitespace-pre-line"><p className="mb-1 text-[10px] font-bold uppercase tracking-[0.16em] opacity-60">Você</p>{message.text}</div> : <div key={message.id}><p className="mb-2 ml-1 text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">FINANZZI</p><FinanceResponseCard text={message.text} /></div>)}
            </div>
          </div>
        </main>

        <footer className="mx-auto w-full max-w-3xl shrink-0 pb-[max(8px,env(safe-area-inset-bottom))] pt-1">
          <div className="flex min-h-5 items-center justify-end px-1 text-[11px] text-muted-foreground">{messages.length > 0 && <button type="button" onClick={() => setMessages([])} className="text-muted-foreground transition hover:text-foreground">Nova conversa</button>}</div>
          <form onSubmit={(event) => { event.preventDefault(); void handleSend(); }} className="mt-1 flex items-end gap-2 rounded-[27px] border border-white/[0.10] bg-card/90 p-2 shadow-[0_16px_50px_rgba(0,0,0,0.28)] backdrop-blur-xl transition-colors focus-within:border-primary/30">
            ...
            <button type="submit" disabled={!input.trim() || busy} className="grid size-11 shrink-0 place-items-center rounded-full bg-primary text-primary-foreground shadow-lg shadow-primary/20 transition-all active:scale-95 disabled:opacity-35" aria-label="Enviar">{busy ? <Loader2 className="size-4 animate-spin" /> : <Send className="size-4" />}</button>
          </form>
        </footer>
      </div>

      {sidebarOpen && <button type="button" aria-label="Fechar menu" className="fixed inset-0 z-[60] bg-black/70 backdrop-blur-sm" onClick={() => setSidebarOpen(false)} />}
      <aside className={cn("fixed inset-y-0 left-0 z-[70] flex w-[min(86vw,320px)] flex-col border-r border-white/[0.08] bg-[#0D0D0D]/95 p-4 pb-[max(16px,env(safe-area-inset-bottom))] pt-[max(16px,env(safe-area-inset-top))] shadow-[20px_0_70px_rgba(0,0,0,0.35)] backdrop-blur-2xl transition-transform duration-300", sidebarOpen ? "translate-x-0" : "-translate-x-full")}>
        <div className="flex items-center justify-between px-2 py-1"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">FINANZZI</p><p className="mt-1 text-[11px] text-muted-foreground">Tudo do seu dinheiro, sem poluição.</p></div><button type="button" onClick={() => setSidebarOpen(false)} aria-label="Fechar menu" className="grid size-10 place-items-center rounded-full hover:bg-card"><ChevronLeft className="size-5" /></button></div>
        <button type="button" onClick={() => { setMessages([]); setInput(""); setSidebarOpen(false); }} className="mt-7 flex items-center gap-3 rounded-2xl border border-white/[0.08] bg-card px-4 py-3 text-left text-sm font-semibold transition hover:border-primary/30"><span className="grid size-8 place-items-center rounded-xl bg-primary text-primary-foreground"><Plus className="size-4" /></span>Nova conversa</button>
        <nav className="mt-6 space-y-1">{nav.map(({ to, label, icon: Icon }) => <Link key={to} to={to} onClick={() => setSidebarOpen(false)} className="flex items-center gap-3 rounded-2xl px-3 py-3.5 text-sm text-muted-foreground transition hover:bg-card hover:text-foreground"><span className="grid size-9 place-items-center rounded-xl bg-card text-primary"><Icon className="size-4" /></span>{label}<ChevronRight className="ml-auto size-4 opacity-40" /></Link>)}</nav>
        <div className="mt-auto rounded-2xl border border-primary/10 bg-primary/[0.05] p-4"><p className="flex items-center gap-2 text-xs font-semibold"><Sparkles className="size-3.5 text-primary" /> Atalhos inteligentes</p><div className="mt-3 space-y-1.5">{suggestions.map((item) => <button key={item} type="button" disabled={busy} onClick={() => { setSidebarOpen(false); useSuggestion(item); }} className="w-full rounded-xl px-2 py-2 text-left text-[11px] text-muted-foreground hover:bg-primary/[0.08] hover:text-foreground">{item}</button>)}</div></div>
      </aside>
    </section>
  );
}
