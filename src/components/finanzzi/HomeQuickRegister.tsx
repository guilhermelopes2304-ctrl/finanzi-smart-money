...
async function findSavedTransaction(userId: string, description: string, amount: number, type: TransactionType) {
  const { data, error } = await supabase.from("transactions").select("id,user_id,description,amount,type,category_id,account_id,credit_card_id,purchase_id,bill_id,date,payment_method,notes,recurrence,installment_number,installment_total").eq("user_id", userId).eq("description", description.trim()).eq("amount", amount).eq("type", type).eq("date", todayISO()).order("date", { ascending: false }).limit(1);
...
export function HomeQuickRegister({ profileName }: Props) {
  ...
  async function register(rawText: string) {
    ...
    try {
      ...
      if (interpretation.intent === "record_transaction") {
        await saveTransaction({ userId: user.id, description: draft.description, amount: draft.amount, type: draft.type, categoryId: draft.categoryId ?? null, accountId: draft.accountId ?? accounts[0]?.id ?? null, cardId: draft.cardId ?? null, date: todayISO(), method: draft.cardId ? "credito" : "pix", notes: "Registrado pela Home", recurrence: "none", ...(draft.installments ? { installments: draft.installments } : {}) });
  ...
  function startVoice() {
    ...
    recognition.onresult = (event) => {
      ...
      if (finalText.trim()) { recognitionRef.current = null; setListening(false); setText(finalText.trim()); void register(finalText.trim()); }
      else if (interimText.trim()) setText(interimText.trim());
    };
    recognition.onerror = (event) => { recognitionRef.current = null; setListening(false); if (event?.error === "not-allowed" || event?.error === "service-not-allowed") toast.error("Permita o acesso ao microfone para registrar por voz."); };
  ...
  async function cancelSaved() {
    ...
    setBusy(true);
    try { await deleteTransaction(user.id, saved.id); setSaved(null); invalidate(); toast.success("Lançamento cancelado", { description: "O valor foi removido dos seus registros." }); }
    catch { toast.error("Não foi possível cancelar o lançamento."); }
    finally { setBusy(false); }
  }

  return <section className="mx-auto w-full max-w-2xl px-4 py-5 sm:px-6 sm:py-8" aria-label="Registro financeiro rápido">
    ...
    {saved && <div className="mt-5 rounded-[24px] border border-primary/20 bg-primary/[0.045] p-4 shadow-sm animate-fin-enter sm:p-5">
      <div className="flex items-start gap-3"><span className={cn("grid size-11 shrink-0 place-items-center rounded-2xl", saved.type === "income" ? "bg-emerald-500/12 text-emerald-400" : "bg-primary/12 text-primary")}><Sparkles className="size-4" /></span><div className="min-w-0 flex-1"><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-primary">Salvo automaticamente</p><p className="mt-1 truncate text-sm font-semibold">{saved.description}</p><p className={cn("mt-1 font-display text-2xl font-semibold tracking-tight", saved.type === "income" ? "text-emerald-400" : "text-foreground")}>{saved.type === "income" ? "+" : "−"}{formatBRL(Number(saved.amount))}</p></div></div>
      <div className="mt-4 grid grid-cols-2 gap-2"><button type="button" onClick={() => setEditOpen(true)} disabled={busy} className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-border bg-background/70 px-3 text-sm font-semibold transition active:scale-[0.98]"><Pencil className="size-4" /> Editar</button><button type="button" onClick={() => void cancelSaved()} disabled={busy} className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-destructive/20 bg-destructive/5 px-3 text-sm font-semibold text-destructive transition active:scale-[0.98]"><Trash2 className="size-4" /> Cancelar</button></div>
    </div>}
