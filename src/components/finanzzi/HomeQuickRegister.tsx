  ...
  async function register(rawText: string) {
    ...
    try {
      ...
      if (interpretation.intent === "record_transaction") {
        ...
        const { data: transaction, error } = await import("@/integrations/supabase/client").then(({ supabase }) =>
          supabase.from("transactions").select("id,user_id,description,amount,type,category_id,account_id,credit_card_id,purchase_id,bill_id,date,payment_method,notes,recurrence,installment_number,installment_total").eq("id", transactionId).eq("user_id", user.id).single(),
        );
        if (error || !transaction) throw new Error(error?.message ?? "Não foi possível confirmar o lançamento");

        setSaved(transaction as Transaction);
        setText("");
        invalidate();
