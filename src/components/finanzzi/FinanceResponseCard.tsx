    ...
    return (
      <article className="w-full max-w-[min(92vw,420px)] rounded-[24px] border border-white/[0.08] bg-card/90 p-4 shadow-[0_14px_45px_rgba(0,0,0,0.24)] backdrop-blur-xl">
        <div className="flex items-start gap-3">
          <div className="grid size-10 shrink-0 place-items-center rounded-2xl bg-primary/10 text-primary"><CheckCircle2 className="size-5" /></div>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2"><p className="text-sm font-semibold text-foreground">{title}</p><span className="rounded-full bg-primary/10 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-primary">Confirmado</span></div>
        ...
        <div className="mt-4 flex gap-2 border-t border-white/[0.07] pt-3">
          <button type="button" disabled={actionBusy} onClick={onEdit} className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-background px-3 py-2.5 text-xs font-medium text-muted-foreground transition hover:text-foreground disabled:opacity-50"><Pencil className="size-3.5" /> Editar</button>
          <button type="button" disabled={actionBusy} onClick={onUndo} className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-background px-3 py-2.5 text-xs font-medium text-muted-foreground transition hover:text-foreground disabled:opacity-50"><RotateCcw className="size-3.5" /> {actionBusy ? "Aguarde..." : "Desfazer"}</button>
        </div>
      </article>
    );
  }
