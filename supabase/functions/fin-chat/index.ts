  ...
  try {
    ...
    const [profileRes, txRes, accountRes, billRes, goalRes] = await Promise.all([
      ...
      supabase
        .from("transactions")
        .select(
          "amount,type,date,description,category_id,account_id,credit_card_id,installment_total,categories(name)",
        )
        .eq("user_id", user.id)
    ...
    const upcomingBills = bills
      .filter((b) => b.status !== "paid" && b.due_date <= monthEnd)
      .reduce((s, b) => s + Number(b.amount || 0), 0);
    const upcomingInstallments = rows
      .filter(
        (t) =>
          t.type === "expense" &&
          t.installment_total &&
    ...
    const recentTransactions = rows.slice(0, 12).map((t) => ({
      ...
      category: Array.isArray(t.categories)
        ? (t.categories[0]?.name ?? null)
        : (t.categories?.name ?? null),
    }));
    ...
    async function callGateway() {
      ...
      if (!res.ok) {
        if (res.status === 402)
          throw new Error(
            "Os créditos de IA do FINANZZI acabaram. Recarregue para continuar conversando com o Fin.",
          );
        if (res.status === 429)
          throw new Error("Muitas perguntas ao mesmo tempo. Tente novamente.");
    ...
    async function callOpenAI() {
      if (!openaiKey) {
        console.error("[fin-chat] OPENAI_API_KEY ausente nos secrets do Supabase");
        throw new Error("OPENAI_API_KEY ausente nos secrets do Supabase");
      }
      ...
      if (!res.ok) {
        const providerBody = (await res.text()).slice(0, 500);
        console.error(`[fin-chat] OpenAI HTTP ${res.status} model=${model}: ${providerBody}`);
        if (res.status === 401)
          throw new Error("A OPENAI_API_KEY foi rejeitada pelo provider OpenAI");
        if (res.status === 403)
          throw new Error("A OPENAI_API_KEY não tem permissão para usar o provider OpenAI");
        if (res.status === 404)
          throw new Error(`O modelo OpenAI ${model} não está disponível para esta chave`);
        if (res.status === 429) throw new Error("O limite ou saldo da conta OpenAI foi atingido");
        throw new Error(`OpenAI indisponível (HTTP ${res.status})`);
      }
      const data = await res.json();
      return String(data?.choices?.[0]?.message?.content ?? "").trim();
    }
    ...
    try {
      ...
      if (m.startsWith("Os créditos") || m.startsWith("Muitas perguntas")) throw e;
